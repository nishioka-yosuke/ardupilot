...
void ModeStar::save_or_move_to_destination(uint8_t dest_num)
{
    // sanity check
    if (dest_num > 1) {
        return;
    ...
    switch (stage) {
        ...
        case STORING_POINTS:
            if (dest_num == 0) {
                ...
                dest_A.x = curr_pos.x;
                dest_A.y = curr_pos.y;
                gcs().send_text(MAV_SEVERITY_INFO, "Star: point A stored");
                copter.Log_Write_Event(DATA_STAR_STORE_A);
            } else {
                // store point B
                dest_B.x = curr_pos.x;
                dest_B.y = curr_pos.y;
                gcs().send_text(MAV_SEVERITY_INFO, "Star: point B stored");
                copter.Log_Write_Event(DATA_STAR_STORE_B);
            }
        ...
        case MANUAL_REGAIN:
            ...
            if (calculate_next_dest(dest_num, stage == AUTO, next_dest, terr_alt)) {
                ...
                if (wp_nav->set_wp_destination(next_dest, terr_alt)) {
                    ...
                    reach_wp_time_ms = 0;
                    if (dest_num == 0) {
                        gcs().send_text(MAV_SEVERITY_INFO, "Star: moving to A");
                    } else {
                        gcs().send_text(MAV_SEVERITY_INFO, "Star: moving to B");
                    }
                }
            }
            break;
...
void ModeStar::return_to_manual_control(bool maintain_target)
{
    if (stage == AUTO) {
        stage = MANUAL_REGAIN;
        loiter_nav->clear_pilot_desired_acceleration();
        ...
        gcs().send_text(MAV_SEVERITY_INFO, "Star: manual control");
    }
}

// fly the vehicle to closest point on line perpendicular to dest_A or dest_B
void ModeStar::auto_control()
{
...
bool ModeStar::reached_destination()
{
    ...
    if (wp_nav->get_wp_distance_to_destination() > STAR_WP_RADIUS_CM) {
        return false;
    }
...
bool ModeStar::calculate_next_dest(uint8_t dest_num, bool use_wpnav_alt, Vector3f& next_dest, bool& terrain_alt) const
{
